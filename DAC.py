...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
	...
	while u > l:
		...
		for i in range(1, int(1001)):  # 1000 iterations is roughly 1 epoch
			data_samples, _ = util.get_mnist_batch('target', batch_size, mnist_train, mnist_train_labels)
		...
		data_samples, data_labels = util.get_mnist_batch('target', batch_size, mnist_test, mnist_test_labels)
		feed_dict={image_pool_input: data_samples}
				   # drop_rate: 0.2,
